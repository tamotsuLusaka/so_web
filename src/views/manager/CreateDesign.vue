  ...
  <div class="_base">
    ...
    <div class="_content" :style="{'padding-bottom': $store.getters['footerHeight'] + 'px'}">
      ...
      <div class="_input">
        ...
        <input type="text" v-model="design.title" id="title" placeholder="" class="_input-text">
      </div>
      <div class="_input">
        <div class="_input-tag">
          <div class="_input-tag-box">
            <input type="radio" id="english" :value="'eg'" v-model="design.language">
            <label for="english" class="_input-tag-label"><span v-if="design.language === 'eg'" class="_input-tag-active"></span></label>
            <label for="english" class="_input-tag-text _eg">English</label>
          </div>
          <div class="_input-tag-box">
            <input type="radio" id="japanese" :value="'jp'" v-model="design.language">
            <label for="japanese" class="_input-tag-label"><span v-if="design.language ==='jp'" class="_input-tag-active"></span></label>
      ...
      <div class="_input">
        ...
        <div class="_input-box">
          ...
          <select v-model="design.tag" required :class="{'_input-select-exist': design.tag !== null}" class="_input-select" >
            <option :value="null" disabled >Select</option>
            <option v-for="tag in $store.getters['tagOfDesign']" :key="tag.text" :value="tag.text" :style="{'color': '#131313'}" >{{tag.text}}</option>
          </select>
        </div>
      </div>
      <div class="_input">
        <label for="date" class="_input-label">Date :</label>
        <input type="number" v-model="design.date" id="date" placeholder="" class="_input-text">
      </div>
      <div class="_input">
        <div class="_input-tag">
          <div class="_input-tag-box">
            <input type="radio" id="public" :value="true" v-model="design.isPublic">
            <label for="public" class="_input-tag-label"><span v-if="design.isPublic" class="_input-tag-active"></span></label>
            <label for="public" class="_input-tag-text _eg">Public</label>
          </div>
          <div class="_input-tag-box">
            <input type="radio" id="unpublic" :value="false" v-model="design.isPublic">
            <label for="unpublic" class="_input-tag-label"><span v-if="!design.isPublic" class="_input-tag-active"></span></label>
      ...
      <router-link :to="{name: 'ManagerOfDesign'}" class="_back"><img src="@/assets/images/arrow-s.png" class="_back-arrow" alt="矢印">Back</router-link>
    </div>
  </div>
...
export default {
  ...
  created(){
    ...
    if(this.$route.params.id){
      ...
      .then((doc)=>{
        ...
        this.design.imageObjects.sort((a,b)=>{
          if(a.order < b.order){return -1}
          if(a.order > b.order){return 1}
          return 0
        })
        this.images = new Images(this.design.imageObjects)
      })
    }else{
      this.mode = "create"
  ...
  methods:{
    async dataMethod(action){
      ...
      const saveObject = {data: this.design, table: "design", afterRouterName: "ManagerOfDesign"}
      if(action === 'create'){
