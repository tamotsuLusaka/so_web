  ...
  <div class="_base">
    ...
    <div class="_content" :style="{'padding-bottom': $store.getters['footerHeight'] + 'px'}">
      ...
      <div class="_input">
        ...
        <input type="text" v-model="brand.title" id="title" placeholder="" class="_input-text">
      </div>
      <div class="_input">
        <div class="_input-tag">
          <div class="_input-tag-box">
            <input type="radio" id="english" :value="'eg'" v-model="brand.language">
            <label for="english" class="_input-tag-label"><span v-if="brand.language === 'eg'" class="_input-tag-active"></span></label>
            <label for="english" class="_input-tag-text _eg">English</label>
          </div>
          <div class="_input-tag-box">
            <input type="radio" id="japanese" :value="'jp'" v-model="brand.language">
            <label for="japanese" class="_input-tag-label"><span v-if="brand.language ==='jp'" class="_input-tag-active"></span></label>
      ...
      <div class="_input">
        ...
        <div class="_input-box">
          ...
          <select v-model="brand.tag" required :class="{'_input-select-exist': brand.tag !== null}" class="_input-select" >
            <option :value="null" disabled >Select</option>
            <option v-for="tag in $store.getters['tagOfBrand']" :key="tag.text" :value="tag.text" :style="{'color': '#131313'}" >{{tag.text}}</option>
          </select>
        </div>
      </div>
      <div class="_input">
        <label for="date" class="_input-label">Date :</label>
        <input type="number" v-model="brand.date" id="date" placeholder="" class="_input-text">
      </div>
      <div class="_input">
        <label for="url" class="_input-label">Url :</label>
        <input type="text" v-model="brand.url" id="url" placeholder="" class="_input-text">
      </div>
      <div class="_input">
        <label for="text" class="_input-label">Text :</label>
        <textarea name="" id="text" v-model="brand.text" placeholder="" class="_input-textarea" ></textarea>
      </div>
      <div class="_input">
        <div class="_input-tag">
          <div class="_input-tag-box">
            <input type="radio" id="public" :value="true" v-model="brand.isPublic">
            <label for="public" class="_input-tag-label"><span v-if="brand.isPublic" class="_input-tag-active"></span></label>
            <label for="public" class="_input-tag-text _eg">Public</label>
          </div>
          <div class="_input-tag-box">
            <input type="radio" id="unpublic" :value="false" v-model="brand.isPublic">
            <label for="unpublic" class="_input-tag-label"><span v-if="!brand.isPublic" class="_input-tag-active"></span></label>
...
export default {
  ...
  created(){
    ...
    if(this.$route.params.id){
      ...
      .then((doc)=>{
        this.brand = doc.data()
        this.brand.imageObjects.sort((a,b)=>{
          if(a.order < b.order){return -1}
          if(a.order > b.order){return 1}
          return 0
        })
        this.images = new Images(this.brand.imageObjects)
      })
    }else{
      this.mode = "create"
  ...
  methods:{
    async dataMethod(action){
      ...
      const saveObject = {data: this.brand, table: "brand", afterRouterName: "ManagerOfBrand"}
      if(action === 'create'){
